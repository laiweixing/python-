...
def runTest(testCaseFile, config):
    ...
    for i in range(1, table.nrows):
        ...
        data = Common.delEnter(table.cell(i, 7).value)          # 获取预期返回的数据：参数

        Log.writeLog(fileName, '第' + str(i) + '条测试数据读取完成')
        url = url + api_host    # 拼接ip和路径,组成请求链接
        data = data.split('~')      # 预期返回数据转化为字典
        ...
        if ipType == serverType.test.value:
            ...
            if method == "get":
                ...
                if state == 200:
                    ...
                    continue
                change_dic_rContennt(response, i, fileName)
                rCode = rContent[rcode]     # 错误码
                rDesc = rContent[rdesc]     # 错误描述
                rData = rContent[rdata]     # 返回的json数据
                ...
                elif returnVal == '返回值=y' and rCode != 1:
                    results = 'code:'+str(rCode)+' '+'desc:'+str(rDesc)

                elif returnVal == '返回值=n' and rCode == 1:
                    results = '数据返回成功'
...
# 字符串转化为字典
def change_dic_rContennt(response, i, fileName):
    try:
        global rContent
        rContent = eval(response.text)
    except Exception as e:
        print(e)
        Log.writeLog(fileName, "第" + str(i) + "条数据中返回的数据非json格式")
        sys.exit()
    else:
        Log.writeLog(fileName, "第" + str(i) + "条数据，rContent转换成功")
        return rContent
